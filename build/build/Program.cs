    ...
    internal static partial class Program
    {
        internal static async Task Main(string[] args)
        {
            static string logOptions(string target)
                => $"/maxcpucount /nologo /verbosity:minimal /bl:./artifacts/logs/corny.{target}.binlog";
            ...
            Target("test", DependsOn("build"), async () =>
            {
                ...
                var tests = tfms
                    .Select(tfm => RunAsync("dotnet", $"run --project test/Xenial.Corny.Tests/Xenial.Corny.Tests.csproj --no-build --no-restore --framework {tfm} -c {Configuration} {properties()}"))
                    .ToArray();
            ...
            Target("pack", DependsOn("lic"),
                () => RunAsync("dotnet", $"pack Xenial.Corny.sln --no-restore --no-build -c {Configuration} {logOptions("pack.nuget")} {properties()}")
            );
